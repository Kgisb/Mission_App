...
if view == "MIS":
    ...
    if not (_create_ok and _pay_ok):
        ...
        def _counts_for_window(start_d: date, end_d: date, mode: str) -> dict:
            ...
            # Referral deals/payments (Deal Source contains 'referral')
            if _src_col:
                is_referral_src = _SRC.str.contains(r"\breferral\b", case=False, na=False)
                referral_deals = int((c_in & is_referral_src).sum()) if mode == "MTD" else int((p_in & is_referral_src).sum())
            else:
                referral_deals = 0

            # Referral sales (from Referral Intent Source): payments with known intent
